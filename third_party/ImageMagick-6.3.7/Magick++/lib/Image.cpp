				      ...
				      const Magick::Image &texture_ )
{
  ...
  target.blue=p->blue;
  if (p)
    PaintFloodfillImage ( image(), // Image *image
                          DefaultChannels,
				     ...
				     const Magick::PaintMethod method_ )
{
  ...
  target.opacity=opacity_;
  PaintFloodfillImage ( image(), OpacityChannel, &target, x_, y_,
    options()->drawInfo(), method_ );
  throwImageException();
}
			     ...
			     const Color &penColor_ )
{
  ...
  PaintOpaqueImage ( image(), &opaque, &pen );
  throwImageException();
}
...
void Magick::Image::transparent ( const Color &color_ )
{
  ...
  modifyImage();
  PaintTransparentImage ( image(), &target, TransparentOpacity );
  throwImageException();
}
...
{
  extern "C" {
    void MagickPlusPlusMagickCoreTerminus(void);
  }
}

void Magick::MagickPlusPlusMagickCoreTerminus(void)
{
  if (magick_initialized)
    {
      magick_initialized=false;
      MagickLib::MagickCoreTerminus();
    }
}
...
{
  MagickLib::MagickCoreGenesis(path_,MagickFalse);
  if (!magick_initialized)
...
Magick::MagickCleanUp::~MagickCleanUp ( void )
{
  MagickPlusPlusMagickCoreTerminus();
}
