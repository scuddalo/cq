...
WandExport PixelIterator *NewPixelIterator(MagickWand *wand)
{
  ...
  if (depth != MAGICKCORE_QUANTUM_DEPTH)
    ThrowWandFatalException(WandError,"MAGICKCORE_QUANTUM_DEPTHMismatch",quantum);
  ...
  unsigned long *number_wands)
{
  ...
  *number_wands=0;
  ...
  p=AcquireCacheViewPixels(iterator->view,iterator->region.x,iterator->region.y+
    iterator->y,iterator->region.width,1,iterator->exception);
  if (p == (const PixelPacket *) NULL)
    return((PixelWand **) NULL);
  indexes=AcquireCacheViewIndexes(iterator->view);
  for (x=0; x < (long) iterator->region.width; x++)
  {
    PixelSetQuantumColor(iterator->pixel_wands[x],p);
