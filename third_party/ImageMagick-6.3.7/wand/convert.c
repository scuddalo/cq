  ...
  ExceptionInfo *exception)
{
  ...
  /*
    Open output file.
  */
  output=MagickOpenStream(argv[argc-1],"wb");
  if (output == (FILE *) NULL)
    {
  ...
  {
    input=MagickOpenStream(argv[i],"rb");
    if (input == (FILE *) NULL)
        ...
        if (LocaleCompare("distort",option+1) == 0)
          {
            ...
            if (op < 0)
              ThrowConvertException(OptionError,"UnrecognizedDistortType",
                argv[i]);
            i++;
            if (i == (long) (argc-1))
              ThrowConvertException(OptionError,"MissingArgument",option);
  ...
  if (metadata != (char **) NULL)
    {
      ...
      text=InterpretImageProperties(image_info,image_stack[k],format);
      if (text == (char *) NULL)
        {
          char
            *message;
