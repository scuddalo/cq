...
MagickExport MagickBooleanType NTGetExecutionPath(char *path,
  const size_t extent)
{
  GetModuleFileName(0,path,extent);
  return(MagickTrue);
}

/*
...
MagickExport char *NTRegistryKeyLookup(const char *subkey)
{
  ...
  */
  size=32;
  value=(char *) AcquireQuantumMemory(size,sizeof(*value));
  if (value == (char *) NULL)
    {
  ...
  if ((status == ERROR_MORE_DATA) && (type == REG_SZ))
    {
      value=(char *) ResizeQuantumMemory(value,size,sizeof(*value));
      if (value == (char *) NULL)
        {
          RegCloseKey(registry_key);
          return((char *) NULL);
...
%
%      int NTTruncateFile(int file,const MagickSizeType length)
%
%  A description of each parameter follows:
%
%    o file: The file.
...
%
*/
MagickExport int NTTruncateFile(int file,const MagickSizeType length)
{
  DWORD
    file_pointer;
