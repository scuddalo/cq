  ...
  const Image *source,const AffineMatrix *affine)
{
  ...
  */
  ...
  GetMagickPixelPacket(image,&composite);
  ...
  source_view=OpenCacheView(source);
  for (y=(long) (edge.y1+0.5); y <= (long) (edge.y2+0.5); y++)
  {
    inverse_edge=AffineEdge(source,&inverse_affine,(double) y+0.5,&edge);
    if (inverse_edge.x2 < inverse_edge.x1)
      continue;
    q=GetCacheViewPixels(image_view,(long) (inverse_edge.x1+0.5),y,
      (unsigned long) ((long) (inverse_edge.x2+0.5)-(long)
      (inverse_edge.x1+0.5)+1),1);
  ...
  source_view=CloseCacheView(source_view);
  ...
  for (q=primitive; *q != '\0'; )
  {
        ...
        if (LocaleCompare("fill-opacity",keyword) == 0)
          {
            ...
            if (graphic_context[n]->fill.opacity != (Quantum) TransparentOpacity)
              graphic_context[n]->fill.opacity=(Quantum)
                ((MagickRealType) QuantumRange*(1.0-factor*atof(token)));
            break;
          }
        ...
        if (LocaleCompare("opacity",keyword) == 0)
          {
            ...
            graphic_context[n]->opacity=RoundToQuantum((MagickRealType)
              QuantumRange*(1.0-((1.0-graphic_context[n]->opacity/
              QuantumRange)*factor*atof(token))));
        ...
        if (LocaleCompare("stroke-opacity",keyword) == 0)
          {
            GetMagickToken(q,&q,token);
            factor=strchr(token,'%') != (char *) NULL ? 0.01 : 1.0;
            if (graphic_context[n]->stroke.opacity != (Quantum) TransparentOpacity)
    ...
    */
    ...
    if (primitive_info->text != (char *) NULL)
      primitive_info->text=(char *)
        RelinquishMagickMemory(primitive_info->text);
    if (image->progress_monitor != (MagickProgressMonitor) NULL)
      {
        status=image->progress_monitor(RenderImageTag,q-primitive,
          (MagickSizeType) primitive_extent,image->client_data);
        if (status == MagickFalse)
          break;
      }
  }
...
static inline MagickRealType GetStopColorOffset(
  const SegmentInfo *gradient_vector,const long x,const long y)
{
  MagickRealType
    gamma,
    length,
      ...
      switch (gradient->spread)
      {
        ...
        case PadSpread:
        {
          if ((x != (long) (gradient->gradient_vector.x1+0.5)) ||
            ...
            offset=GetStopColorOffset(&gradient->gradient_vector,x,y)/length;
        ...
        case RepeatSpread:
        {
          ...
          if ((x != (long) (gradient->gradient_vector.x1+0.5)) ||
              (y != (long) (gradient->gradient_vector.y1+0.5)))
            {
              offset=GetStopColorOffset(&gradient->gradient_vector,x,y);
  ...
  if (primitive_info->coordinates == 1)
    {
      ...
      */
      ...
      {
        x=start;
        q=GetCacheViewPixels(image_view,x,y,(unsigned long) (stop-x+1),1);
        if (q == (PixelPacket *) NULL)
          break;
        for ( ; x <= stop; x++)
        {
          if ((x == (long) (primitive_info->point.x+0.5)) &&
            ...
            *q=GetStrokeColor(draw_info,x,y);
          q++;
        }
        if (SyncCacheView(image_view) == MagickFalse)
          break;
    ...
    for ( ; x <= stop; x++)
    {
      ...
      */
      ...
      MagickCompositeOver(&fill_color,fill_opacity,q,(MagickRealType)
        q->opacity,q);
      stroke_color=GetStrokeColor(draw_info,x,y);
  ...
  const DrawInfo *draw_info,const PrimitiveInfo *primitive_info)
{
  ...
  switch (primitive_info->primitive)
  {
    case PointPrimitive:
    {
      ...
      q=GetCacheViewPixels(image_view,x,y,1,1);
      if (q == (PixelPacket *) NULL)
        break;
      fill_color=GetFillColor(draw_info,x,y);
    ...
    case ColorPrimitive:
    {
      switch (primitive_info->method)
      {
        ...
        case ReplaceMethod:
        {
          ...
          target=AcquireOneCacheViewPixel(image_view,x,y,&image->exception);
        ...
        case FillToBorderMethod:
        {
          ...
          target=AcquireOneMagickPixel(image,x,y,&image->exception);
          ...
          (void) PaintFloodfillImage(image,DefaultChannels,&target,x,y,
            draw_info,primitive_info->method);
          break;
        }
        case ResetMethod:
        {
    ...
    case MattePrimitive:
    {
      ...
      switch (primitive_info->method)
      {
        ...
        default:
        {
          ...
          if (q == (PixelPacket *) NULL)
            break;
          q->opacity=GetFillColor(draw_info,x,y).opacity;
          (void) SyncCacheView(image_view);
          break;
        }
        case ReplaceMethod:
        ...
        case FillToBorderMethod:
        {
          ...
          (void) PaintFloodfillImage(image,OpacityChannel,&target,x,y,
            draw_info,primitive_info->method);
          break;
        }
        case ResetMethod:
    ...
    case ImagePrimitive:
    {
      ...
      if (composite_image == (Image *) NULL)
        break;
      (void) SetImageProgressMonitor(composite_image,
