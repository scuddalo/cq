...
static const char
  *ColorMap = (const char *)
    "<?xml version=\"1.0\"?>"
    "<colormap>"
    "  <color name=\"none\" color=\"rgb(0,0,0,0)\" compliance=\"SVG\" />"
    ...
    "  <color name=\"opaque\" color=\"rgba(0,0,0)\" compliance=\"SVG\" />"
    ...
    "  <color name=\"transparent\" color=\"rgb(0,0,0,0)\" compliance=\"SVG\" />"
...
#define SVGCompliant(component) ((MagickRealType) \
  ...
  if ((color.matte != MagickFalse) && (color.opacity != OpaqueOpacity))
  ...
  if ((color.matte != MagickFalse) && (color.opacity != OpaqueOpacity))
    {
      (void) ConcatenateMagickString(tuple,",",MaxTextExtent);
      ConcatenateColorComponent(&color,OpacityChannel,SVGCompliance,tuple);
    }
  (void) ConcatenateMagickString(tuple,")",MaxTextExtent);
  LocaleLower(tuple);
  return;
...
#define HistogramImageTag  "Histogram/Image"
  ...
  for (i=0; i < (long) number_colors; i++)
  {
    ...
    (void) QueryMagickColorname(image,&pixel,SVGCompliance,MagickFalse,
      color,exception);
    (void) QueryMagickColorname(image,&pixel,SVGCompliance,MagickTrue,hex,
      exception);
  ...
  ExceptionInfo *exception)
{
  ...
  switch (image->storage_class)
  {
    ...
    case PseudoClass:
    {
      ...
      return(i < (long) image->colors ? MagickFalse : MagickTrue);
    }
  }
  return(MagickTrue);
}
...
#define SearchImageText  "  Searching image...  "
  ...
  GetMagickPixelPacket(image,&target);
  image_view=OpenCacheView(image);
  target_view=OpenCacheView(target_image);
  for (y=(*y_offset); y < (long) image->rows; y++)
  {
    for (x=y == 0 ? *x_offset : 0; x < (long) image->columns; x++)
    {
      for (j=0; j < (long) target_image->rows; j++)
      {
        ...
        {
          p=AcquireCacheViewPixels(image_view,x+i,y+j,1,1,exception);
          indexes=AcquireCacheViewIndexes(image_view);
          SetMagickPixelPacket(image,p,indexes,&pixel);
          q=AcquireCacheViewPixels(target_view,i,j,1,1,exception);
          target_indexes=AcquireCacheViewIndexes(target_view);
  ...
  target_view=CloseCacheView(target_view);
  image_view=CloseCacheView(image_view);
  *x_offset=x;
  *y_offset=y;
  ...
  const MagickPixelPacket *q)
{
  ...
  if (p->fuzz == 0.0)
    fuzz=3.0*MagickMax(q->fuzz,MagickSQ1_2)*MagickMax(q->fuzz,MagickSQ1_2);
  else
    if (q->fuzz == 0.0)
  ...
  if (p->colorspace == CMYKColorspace)
    {
      pixel=alpha*p->index-beta*q->index;
      distance+=pixel*pixel;
      if (distance > fuzz)
        return(MagickFalse);
    }
  ...
  ExceptionInfo *exception)
{
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      if ((p->red != p->green) || (p->green != p->blue) || ((p->red != 0) &&
          (p->red != (Quantum) QuantumRange)))
        break;
      if ((p->opacity != (Quantum) OpaqueOpacity) &&
  ...
  const unsigned long depth,ExceptionInfo *exception)
{
  ...
  */
  ...
  color_map=NewXMLTree(xml,exception);
  if (color_map == (XMLTreeInfo *) NULL)
    return(MagickFalse);
  status=MagickTrue;
  include=GetXMLTreeChild(color_map,"include");
  ...
  color=GetXMLTreeChild(color_map,"color");
  while (color != (XMLTreeInfo *) NULL)
  {
    const char
      *attribute;
    ...
    */
    ...
    GetMagickPixelPacket((const Image *) NULL,&color_info->color);
    color_info->path=ConstantString(filename);
    color_info->signature=MagickSignature;
    attribute=GetXMLTreeAttribute(color,"name");
    if (attribute != (const char *) NULL)
      color_info->name=ConstantString(attribute);
    attribute=GetXMLTreeAttribute(color,"color");
    if (attribute != (const char *) NULL)
      (void) QueryMagickColor(attribute,&color_info->color,exception);
    attribute=GetXMLTreeAttribute(color,"compliance");
    if (attribute != (const char *) NULL)
      {
        long
          compliance;
        ...
        if (GlobExpression(attribute,"*SVG*",MagickTrue) != MagickFalse)
          compliance|=SVGCompliance;
        if (GlobExpression(attribute,"*X11*",MagickTrue) != MagickFalse)
          compliance|=X11Compliance;
        if (GlobExpression(attribute,"*XPM*",MagickTrue) != MagickFalse)
          compliance|=XPMCompliance;
        color_info->compliance=(ComplianceType) compliance;
      }
    attribute=GetXMLTreeAttribute(color,"stealth");
    if (attribute != (const char *) NULL)
      color_info->stealth=IsMagickTrue(attribute);
    status=AppendValueToLinkedList(color_list,color_info);
    if (status == MagickFalse)
      (void) ThrowMagickException(exception,GetMagickModule(),
        ResourceLimitError,"MemoryAllocationFailed","`%s'",filename);
    color=GetNextXMLTreeTag(color);
  }
  color_map=DestroyXMLTree(color_map);
  return(status);
}

/*
  ...
  ExceptionInfo *exception)
{
  ...
  status=QueryMagickColorname(image,&pixel,compliance,MagickFalse,name,
    exception);
  return(status);
}

  ...
  if (*name == '#')
    {
      ...
      */
      ...
      color->red=(MagickRealType) ScaleAnyToQuantum(pixel.red,depth);
      color->green=(MagickRealType) ScaleAnyToQuantum(pixel.green,depth);
      color->blue=(MagickRealType) ScaleAnyToQuantum(pixel.blue,depth);
      color->opacity=(MagickRealType) OpaqueOpacity;
      if ((n % 3) != 0)
        {
          color->matte=MagickTrue;
          color->opacity=(MagickRealType) (QuantumRange-
            ScaleAnyToQuantum(pixel.opacity,depth));
        }
      color->index=0.0;
      return(MagickTrue);
    }
  if (strchr(name,'(') != (char *) NULL)
    {
      ...
      */
      ...
      if ((flags & SigmaValue) != 0)
        color->green=(MagickRealType) RoundToQuantum(scale*
          geometry_info.sigma);
      if ((flags & XiValue) != 0)
...
%        const PixelPacket *color,const ComplianceType compliance,
%        const MagickBooleanType hex,char *name,ExceptionInfo *exception)
%
%  A description of each parameter follows.
%
%    o image: The image.
  ...
  if (node_info->level == (MaxTreeDepth-1))
    {
      ...
      for (i=0; i < (long) node_info->number_unique; i++)
      {
        q=SetImagePixels(image,cube_info->x,0,1,1);
