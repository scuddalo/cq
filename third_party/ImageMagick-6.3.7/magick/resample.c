  ...
  ResampleFilter *resample_filter,const double u0,const double v0)
{
  ...
  if ( resample_filter->do_interpolate ) {
    pixel = InterpolateResampleFilter(resample_filter,
         resample_filter->interpolate,u0,v0);
    return(pixel);
  }
  ...
  */
  ...
  for( v=v1; v<=v2;  v++, u1+=resample_filter->slope ) {
    ...
    pixels=AcquireCacheViewPixels(resample_filter->view,u,v,1UL*uw,1,
      resample_filter->exception);
    if (pixels == (const PixelPacket *) NULL)
      return(pixel);
    indexes=AcquireCacheViewIndexes(resample_filter->view);
  ...
  const FilterTypes filter,const double blur)
{
  ...
  /* Scale radius so it equals 1.0, at edge of ellipse when a
     ...
     table.  This may require some scaling such as  x = sqrt(Q*PI/2048)
     Note PI/2  is about 1.7 which seems to produce a better result.
  */
  ...
  default:
    /*
      ...
      But make teh Sinc/Bessel tapered window 2.0
    ...
    */
    ...
    if (resize_filter != (ResizeFilter *) NULL) {
      ...
      if ( resample_filter->support >= (double)WLUT_WIDTH )
           resample_filter->support = (double)WLUT_WIDTH;
