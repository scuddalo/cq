...
struct _ResizeFilter
{
  MagickRealType
    ...
    support,   /* filter regaion of support */
    blur,      /* x-scale (blur-sharpen) */
    wscale,      /* x-scale (blur-sharpen) */
    cubic[8];  /* cubic coefficents */
...
%         The GetResizeFilterWeight() ensures this is a positive value.
%
...
static MagickRealType Blackman(const MagickRealType x,
     const ResizeFilter *magick_unused(resize_filter))
{
  /* 2rd Order cosine windowing function */
...
static MagickRealType Box(const MagickRealType magick_unused(x),
     const ResizeFilter *magick_unused(resize_filter))
{
  /* Just return 1.0, it is clipped by its support window */
  return(1.0);
}

#if 0
/* Replaced by CubicBC below */
static MagickRealType Catrom(const MagickRealType x,
     const ResizeFilter *magick_unused(resize_filter))
{
  /* Catmull-Rom or Keys  Cubic Filter,  B=0  C=1/2 */
  /* This is the Cubic Spline Interpolator */
  if (x < 1.0)
...
static MagickRealType Cubic(const MagickRealType x,
     const ResizeFilter *magick_unused(resize_filter))
{
  /* B-Spline, B=1  C=0, A Cubic Spline approximation of Gaussian */
  if (x < 1.0)
     ...
     const ResizeFilter *resize_filter)
{
  /*
    ...
    Which ensures function is continuious in value and derivative (slope)
   */
  if (x < 1.0)
    return(resize_filter->cubic[0] +x*(resize_filter->cubic[1]
              +x*(resize_filter->cubic[2] +x*resize_filter->cubic[3] )));
  if (x < 2.0)
    return(resize_filter->cubic[4] +x*(resize_filter->cubic[5]
              +x*(resize_filter->cubic[6] +x*resize_filter->cubic[7] )));
  return(0.0);
}
...
#if 0
/* Replaced by CubicBC above */
static MagickRealType Hermite(const MagickRealType x,
     const ResizeFilter *magick_unused(resize_filter))
{
  /* A Cubic windowing function,  B=0, C=0 */
  if (x < 1.0)
...
#endif
{
  /* Piece-wise polynomial fit of Sinc */
  /* This filter is not yet complete -- Currently only the Cubic form */
#if 1
  if (x < 1.0)
  ...
  return(0.0);
#else
  int terms = (int)(resize_filter->support*2-1);
  int piece = 0;
  if ( x > (MagickRealType)terms/2 )
    return(0.0);
#endif
}
...
static MagickRealType Lanczos(const MagickRealType x,
     const ResizeFilter *magick_unused(resize_filter))
{
  /* The lanzcos, sinc windowed sinc Filter
     But it also ise used as a bessel windowed bessel
     which complicates the use of the filter.
  */
  if (x < support)
  ...
  const MagickBooleanType cylindrical, ExceptionInfo *exception)
{
  ...
  /*
    Mapping of FilterTypes  into Filter Weighting and Filter Windowing
    functions.   A 'Box' windowing function means the filter is purely clipped
    to the support window.  A 'Sinc' filter function could be upgraded to a
    'Bessel' filter if a "cylindrical" filter is requested, (with the
    exception of a specific "Sinc" filter request).
  */
  static struct {
    FilterTypes
      filter,
  ...
  } const mapping[SentinelFilter] =
  {
    ...
    { CubicFilter,     BoxFilter },      /* Cubic spline of Gaussian */
    ...
    { SincFilter,      KaiserFilter },   /* Kaiser --  ???-Sinc */
  ...
  */
  static struct {
    MagickRealType
      ...
      B,C;      /* Cubic Filter factors for this function */
  } const filters[SentinelFilter] =
  {
    ...
    { Hanning,   1.0f,  1.0f, 0.0f, 0.0f }, /* \                        */
    { Hamming,   1.0f,  1.0f, 0.0f, 0.0f }, /*  >-  windowing functions */
    { Blackman,  1.0f,  1.0f, 0.0f, 0.0f }, /* /                        */
    { Gaussian,  1.5f,  1.5f, 0.0f, 0.0f },
    { Quadratic, 1.5f,  1.5f, 0.0f, 0.0f },
    ...
    { Kaiser,    1.0f,  1.0f, 0.0f, 0.0f }, /* \                       */
    { Welsh,     1.0f,  1.0f, 0.0f, 0.0f }, /*  >- windowing functions */
  ...
  */
  ...
  /* Filter blur -- scale both filter and support window */
  artifact=GetImageArtifact(image,"filter:blur");
  if (artifact != (const char *) NULL)
    resize_filter->blur = atof(artifact);
  if ( resize_filter->blur < MagickEpsilon )
    resize_filter->blur = MagickEpsilon;

  /* Set Cubic Spline Coefficents - from B,C values
     See CubicBC() function above */
  if ( filters[filter_type].function == CubicBC
    ...
    if (artifact != (const char *) NULL) {
      B=atof(artifact);
      C=1.0-2.0*B;  /* Assume its a Keys type filter - unles C is also given */
    ...
    /* Pre-define B,C into Cubic Coefficents - See Cubic() */
  ...
  if (artifact != (const char *) NULL) {
    MagickRealType x;
    MagickRealType s = GetResizeFilterSupport(resize_filter);

    printf("# support = %lg\n", (double) s);
    for ( x = 0.0; x <= s; x+=0.01f )
      printf("%5.2lf %lf\n", (double) x, (double) GetResizeFilterWeight(resize_filter, x) );
    printf("%5.2lf %lf\n", (double) s, 0.0 );
  }
#endif
  ...
  const ResizeFilter *resize_filter,const MagickRealType x)
{
  ...
  x_blur  = fabs(x)/resize_filter->blur;
  wscale  = resize_filter->wscale;
  support = resize_filter->support;
  if ( support < MagickEpsilon )
    wscale = 1.0; /* Point Filter -- avoid division by zero */
  else
    wscale  = resize_filter->window(x_blur*wscale/support,resize_filter);
  return(wscale*resize_filter->filter(x_blur,resize_filter));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
