...
MagickExport ExceptionInfo *DestroyExceptionInfo(ExceptionInfo *exception)
{
  ...
  exception->signature=(~MagickSignature);
  RelinquishSemaphoreInfo(exception->semaphore);
  exception->semaphore=DestroySemaphoreInfo(exception->semaphore);
...
%  A description of each parameter follows:
%
%    o error_code: the error code.
%
*/
MagickExport char *GetExceptionMessage(const int error_code)
{
  return(ConstantString(strerror(error_code)));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  const char *format,va_list operands)
{
  ...
  (void) ThrowException(exception,severity,reason,(char *) NULL);
  return(status);
}
