  ...
  *ImageCompositeHelp[]=
  {
    ...
    "for the image.  This is the case when matte is opaque (full",
  ...
  *DisplayHelp[]=
  {
    ...
    "            this command with a number.  For example to display the",
    "            third page preceeding the current page, press 3<backspace>.",
    "",
  ...
  *ImageMatteEditHelp[]=
  {
    ...
    "This is the case when matte is opaque (full coverage) for",
        ...
        */
        ...
        switch (method)
        {
          ...
          case FillToBorderMethod:
          {
            ...
            */
            target=AcquireOneMagickPixel(*image,x_offset,y_offset,
              &(*image)->exception);
            ...
            (void) PaintFloodfillImage(*image,DefaultChannels,&target,x_offset,
              y_offset,draw_info,method);
        ...
        case KeyPress:
        {
          ...
          */
          ...
          switch ((int) key_symbol)
          {
            ...
            case XK_KP_Home:
            {
              crop_info.x=windows->image.width/2L-crop_info.width/2L;
              crop_info.y=windows->image.height/2L-crop_info.height/2L;
              break;
            }
            case XK_Left:
            case XK_KP_Left:
  ...
  switch (command)
  {
    ...
    case ContrastStretchCommand:
    {
      ...
      */
      XSetCursorState(display,windows,MagickTrue);
      XCheckRefreshWindows(display,windows);
      (void) ContrastStretchImage(*image,levels);
      XSetCursorState(display,windows,MagickFalse);
      if (windows->image.orphan != MagickFalse)
        break;
      XConfigureImageColormap(display,resource_info,windows,*image);
    ...
    case MapCommand:
    {
      ...
      */
      ...
      map_image=ReadImage(image_info,&(*image)->exception);
      ...
      */
      ...
      if (status == MagickFalse)
      ...
      else
        {
          ...
          (void) FormatMagickString(command,MaxTextExtent,"@%s",
            image_info->filename);
          (void) DeleteImageProperty(*image,"comment");
          (void) SetImageProperty(*image,"comment",command);
          (*image)->taint=MagickTrue;
        }
    ...
    case ShowPreviewCommand:
    {
      ...
      */
      ...
      (void) FormatMagickString((*image)->filename,MaxTextExtent,"preview:%s",
        filename);
      status=WriteImage(image_info,*image);
      (void) FormatMagickString((*image)->filename,MaxTextExtent,"show:%s",
        filename);
      status=WriteImage(image_info,*image);
      if (status == MagickFalse)
        ...
        switch (method)
        {
          ...
          case FillToBorderMethod:
          {
            ...
            */
            ...
            (void) PaintFloodfillImage(*image,OpacityChannel,&target,x_offset,
              y_offset,draw_info,method);
            draw_info=DestroyDrawInfo(draw_info);
            break;
          }
        ...
        case KeyPress:
        {
          ...
          */
          ...
          switch ((int) key_symbol)
          {
            ...
            case XK_KP_Home:
            {
              roi_info.x=windows->image.width/2L-roi_info.width/2L;
              roi_info.y=windows->image.height/2L-roi_info.height/2L;
              break;
            }
            case XK_Left:
            case XK_KP_Left:
    ...
    if (next_image != (Image *) NULL)
      {
        ...
        (void) ParseSizeGeometry(next_image,read_info->size,&geometry);
        ...
        if ((images->progress_monitor != (MagickProgressMonitor) NULL) &&
            (QuantumTick(i,(MagickSizeType) number_files) != MagickFalse))
          {
...
#define MagickTitle  "Commands"
  ...
  */
  ...
  class_hints->res_class=(char *) resource_info->client_name;
  ...
  */
  ...
  if (resource_info->name == (char *) NULL)
    class_hints->res_name=(char *) resource_info->client_name;
  else
    class_hints->res_name=(char *) resource_info->name;
  manager_hints->flags=IconWindowHint | InputHint | StateHint;
  manager_hints->icon_window=windows->icon.id;
  manager_hints->input=MagickTrue;
  manager_hints->initial_state=
