...
MagickExport Image *CoalesceImages(const Image *image,ExceptionInfo *exception)
{
  ...
  */
  ...
  for ( ; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    ...
    */
    prev=GetPreviousImageInList(next);
    bounds=prev->page;
    bounds.width=prev->columns;
    bounds.height=prev->rows;
    if (bounds.x < 0)
      {
        bounds.width+=bounds.x;
        bounds.x=0;
    ...
    */
    if (IsBoundsCleared(prev,coalesce_image,&bounds,exception) )
      coalesce_image->dispose=BackgroundDispose;
    else
      coalesce_image->dispose=NoneDispose;
    prev->dispose=coalesce_image->dispose;
  }
  dispose_image=DestroyImage(dispose_image);
...
MagickExport Image *DisposeImages(const Image *image,ExceptionInfo *exception)
{
  ...
  */
  ...
  for (; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    ...
    */
    {
      Image *dispose=CloneImage(dispose_image,0,0,MagickTrue,exception);
  ...
  const ImageLayerMethod method, ExceptionInfo *exception)
{
  ...
  */
  ...
  bounds=(RectangleInfo *) AcquireQuantumMemory((size_t)
    GetImageListLength(next)-1UL,sizeof(*bounds));
...
%  is as yet incomplete, or it is not a GIF animation.  This is a non-sensible
...
%  coalesced before use.  However this is not a requirement, and individual
...
#define MergeLayersTag  "Merge/Layers"
    ...
    default:
    {
      ...
      for ( ; next != (Image *) NULL;  next=GetNextImageInList(next)) {
        if ( page.x > next->page.x ) {
             width += next->page.x-page.x;
             page.x = next->page.x;
        }
        if ( page.y > next->page.y ) {
             height += next->page.y-page.y;
             page.y = next->page.y;
        }
      if ( width < next->page.x + next->columns - page.x )
           width = next->page.x + next->columns - page.x;
        if ( height < next->page.y + next->rows - page.y )
             height = next->page.y + next->rows - page.y;
      }
      break;
    }
    case FlattenLayer:
      ...
      for (next=image; next != (Image *) NULL; next=GetNextImageInList(next)) {
        ...
        page.x=next->page.x;
          ...
          if ( width < next->page.x + next->columns )
               width = next->page.x + next->columns;
          if ( height < next->page.y + next->rows )
             height = next->page.y + next->rows;
        }
      }
      page.width=width;
      page.height=height;
  ...
  */
  ...
  {
    (void) CompositeImage(canvas,image->compose,image,
         image->page.x-canvas->page.x, image->page.y-canvas->page.y );
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        (QuantumTick(scene,number_images) != MagickFalse))
      {
        status=image->progress_monitor(MergeLayersTag,scene,number_images,
          image->client_data);
