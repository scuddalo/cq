  ...
  const XPixelInfo *pixel,XAnnotateInfo *annotate_info,Image *image)
{
  ...
  */
  ...
  for (y=0; y < (int) annotate_image->rows; y++)
  {
    ...
    if (SyncImagePixels(annotate_image) == MagickFalse)
      break;
  }
...
%  particular window and updates the area associated with the exposure event.
%
...
MagickExport void XDestroyResourceInfo(XResourceInfo *resource_info)
{
  ...
  if (resource_info->image_info != (ImageInfo *) NULL)
    resource_info->image_info=DestroyImageInfo(resource_info->image_info);
  ...
  const XPixelInfo *pixel,XDrawInfo *draw_info,Image *image)
{
  ...
  */
  ...
  for (y=0; y < (int) draw_image->rows; y++)
  {
    ...
    if (SyncImagePixels(draw_image) == MagickFalse)
      break;
  }
  ...
  const char *client_name)
{
  ...
  */
  ...
  (void) FormatMagickString(filename,MaxTextExtent,"%s%c%s",X11_APPLICATION_PATH,
    c,client_name+1);
  ...
  */
  ...
  (void) FormatMagickString(filename,MaxTextExtent,"%s%src",X11_PREFERENCES_PATH,
    client_name);
...
%
%      void XGetResourceInfo(XrmDatabase database,const char *client_name,
...
%
*/
MagickExport void XGetResourceInfo(XrmDatabase database,const char *client_name,
  XResourceInfo *resource_info)
{
  char
    *resource_value;
  ...
  */
  ...
  resource_info->image_info=AcquireImageInfo();
  ...
  resource_value=XGetResourceClass(database,client_name,"magnify",(char *) "3");
      ...
      */
      ...
      for (id=0; id < number_windows; id++)
      {
        ...
        */
        ...
        switch (composite_image->storage_class)
        {
          ...
          case PseudoClass:
          {
            /*
              Create colormap.
            */
            if (AllocateImageColormap(composite_image,number_colors) == MagickFalse)
              {
                XDestroyImage(ximage);
        ...
        XDestroyImage(ximage);
        composite_image->depth=GetImageDepth(composite_image,
  ...
  else
    {
      ...
      */
      ...
      if (window->name == (char *) NULL)
        window->name=AcquireString("\0");
      if (window->icon_name == (char *) NULL)
        window->icon_name=AcquireString("\0");
      ...
      if ((window->immutable == MagickFalse) &&
          ...
          (window->pixel_info->colors == 0))
        {
          ...
          (void) CopyMagickString(image_info->filename,
            ...
            MaxTextExtent);
          pattern=ReadImage(image_info,&window->image->exception);
          ...
          if (pattern != (Image *) NULL)
            {
              ...
              /*
                Composite transparent image on canvas.
              */
              canvas=CloneImage(window->image,window->image->columns,
                window->image->rows,MagickTrue,&window->image->exception);
              if (canvas != (Image *) NULL)
                {
                  (void) TextureImage(canvas,pattern);
                  (void) CompositeImage(canvas,AtopCompositeOp,window->image,
                    0,0);
                  if (window->image != image)
                    window->image=DestroyImage(window->image);
                  window->image=canvas;
...
MagickExport void XMakeMagnifyImage(Display *display,XWindows *windows)
{
  ...
  */
  pixel=AcquireOneMagickPixel(windows->image.image,windows->magnify.x,
  ...
  (void) QueryMagickColorname(windows->image.image,&pixel,SVGCompliance,
    MagickTrue,tuple,&windows->image.image->exception);
  y+=height;
  (void) XDrawImageString(display,windows->magnify.pixmap,
    windows->magnify.annotate_context,x,y,tuple,(int) strlen(tuple));
  (void) QueryMagickColorname(windows->image.image,&pixel,SVGCompliance,
    MagickFalse,tuple,&windows->image.image->exception);
  y+=height;
  (void) XDrawImageString(display,windows->magnify.pixmap,
    windows->magnify.annotate_context,x,y,tuple,(int) strlen(tuple));
  /*
            ...
            if (q != (PixelPacket *) NULL)
              {
                for (i=0; i < (long) number_colors; i++)
                {
                  q->red=(Quantum) 0;
                  if (map_info->red_max != 0)
                    q->red=ScaleXToQuantum(1UL*i/map_info->red_mult,
                      map_info->red_max);
                  q->green=(Quantum) 0;
                  if (map_info->green_max != 0)
                    q->green=ScaleXToQuantum((1UL*i/map_info->green_mult) %
                      (map_info->green_max+1),map_info->green_max);
                  q->blue=(Quantum) 0;
                  if (map_info->blue_max != 0)
                    q->blue=ScaleXToQuantum(1UL*i % map_info->green_mult,
                      map_info->blue_max);
                  q->opacity=(Quantum) TransparentOpacity;
                  q++;
                }
                (void) SyncImagePixels(map_image);
                (void) MapImage(image,map_image,MagickTrue);
              }
            XGetPixelPacket(display,visual_info,map_info,resource_info,image,
              pixel);
