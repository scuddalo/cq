...
#define SegmentImageTag  "Segment/Image"
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        (QuantumTick(y,2*image->rows) != MagickFalse))
      {
        status=image->progress_monitor(SegmentImageTag,y,2*image->rows,
          image->client_data);
  ...
  */
  if (AllocateImageColormap(image,number_clusters) == MagickFalse)
    ThrowBinaryException(ResourceLimitError,"MemoryAllocationFailed",
      image->filename);
  cluster=head;
  for (i=0; i < (long) number_clusters; i++)
  {
...
%
%      MagickPixelPacket GetImageDynamicThreshold(const Image *image,
