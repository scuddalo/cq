  ...
  */
  ...
  for (p=utf8; *p != '\0'; p++)
  {
    if ((i+MaxTextExtent) > (long) extent)
      {
        extent+=MaxTextExtent;
        canonical_content=(char *) ResizeQuantumMemory(canonical_content,
    ...
    if ((size_t) (j+MaxTextExtent) > max_length)
      {
        ...
        utf8=(char *) ResizeQuantumMemory(utf8,max_length,sizeof(*utf8));
        if (utf8 == (char *) NULL)
          return(utf8);
      }
    if (c < 0x80)
...
%
%      XMLTreeInfo *XMLTreeInfoToXML(XMLTreeInfo *xml_info)
%
%  A description of each parameter follows:
%
%    o xml_info: The xml info.
  ...
  MagickBooleanType pedantic)
{
  ...
  if (canonical_content == (char *) NULL)
    return(*destination);
  if ((*length+strlen(canonical_content)+MaxTextExtent) > *max_length)
    {
      *max_length=(*length)+strlen(canonical_content)+MaxTextExtent;
      *destination=(char *) ResizeQuantumMemory(*destination,*max_length,
        sizeof(**destination));
      if (*destination == (char *) NULL)
        return(*destination);
    }
  *length+=FormatMagickString(*destination+(*length),*max_length,"%s",
    canonical_content);
  ...
  size_t *max_length,size_t start,char ***attributes)
{
  ...
  if ((*length+strlen(xml_info->tag)+MaxTextExtent) > *max_length)
    {
      ...
      *source=(char *) ResizeQuantumMemory(*source,*max_length,sizeof(*source));
      if (*source == (char *) NULL)
        return(*source);
    }
    ...
    if ((*length+strlen(xml_info->attributes[i])+MaxTextExtent) > *max_length)
      {
        ...
        *source=(char *) ResizeQuantumMemory(*source,*max_length,
          sizeof(**source));
        if (*source == (char *) NULL)
          return((char *) NULL);
      }
  ...
  *length+=FormatMagickString(*source+(*length),*max_length,*xml_info->content ?
    ">" : "/>");
  if (xml_info->child != (XMLTreeInfo *) NULL)
  ...
  while ((content[offset] != '\0') && (offset < xml_info->offset))
    offset++;
  if (xml_info->ordered != (XMLTreeInfo *) NULL)
    content=XMLTreeTagToXML(xml_info->ordered,source,length,max_length,offset,
      attributes);
  else
        ...
        if ((length+strlen(p)+strlen(q)+MaxTextExtent) > max_length)
          {
            ...
            xml=(char *) ResizeQuantumMemory(xml,max_length,sizeof(*xml));
            if (xml == (char *) NULL)
              return(xml);
          }
