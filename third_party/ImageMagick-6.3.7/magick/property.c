    ...
    default:
    {
      ...
      */
      for (i=0; ; i++)
      {
        if (Tags[i].tag == 0)
          break;
        if (LocaleCompare(Tags[i].description,property) == 0)
          {
            tag=(unsigned long) Tags[i].tag;
            break;
          }
      }
      break;
  ...
  const char *property)
{
  ...
  for (i=0; (i < (long) GetStringInfoLength(profile)) && (*p != '\0'); i++)
    p++;
  ...
  if (xmp == (XMLTreeInfo *) NULL)
    return(MagickFalse);
  status=MagickFalse;
  child=GetXMLTreeChild(xmp,"rdf:RDF");
  if (child != (XMLTreeInfo *) NULL)
    {
      child=GetXMLTreeChild(child,"rdf:Description");
      if (child != (XMLTreeInfo *) NULL)
        {
          if (image->properties == (void *) NULL)
            ((Image *) image)->properties=NewSplayTree(CompareSplayTreeString,
              RelinquishMagickMemory,RelinquishMagickMemory);
          status=GetXMLTreeAttributes(child,(SplayTreeInfo *)
            image->properties);
        }
    }
...
static const char *GetMagickProperty(const ImageInfo *image_info,Image *image,
  const char *property)
{
  char
    value[MaxTextExtent],
  ...
  Image *image,const char *embed_text)
{
  ...
  if ((*text == '@') && ((*(text+1) == '-') ||
      (IsAccessible(text+1) != MagickFalse)))
    ...
    switch (*p)
    {
      ...
      case 'r':
      {
        ...
        */
        ...
        q+=FormatMagickString(q,extent,"%s%s%s",
      ...
      case 'z':
      {
        /*
          Unique filename.
        */
        q+=FormatMagickString(q,extent,"%lu",GetImageDepth(image,
          &image->exception));
        break;
      }
      case 'C':
