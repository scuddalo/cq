  ...
  const PointInfo *offset,TypeMetric *metrics)
{
  ...
  if (type_info == (const TypeInfo *) NULL)
    {
      status=RenderFreetype(image,draw_info,(const char *) NULL,offset,
        metrics);
      return(status);
    }
  annotate_info=CloneDrawInfo((ImageInfo *) NULL,draw_info);
    ...
    if (draw_info->render != MagickFalse)
      {
        ...
        */
        ...
        for (y=0; y < (long) bitmap->bitmap.rows; y++)
        {
          ...
          q=GetCacheViewPixels(image_view,x_offset,y_offset,
          ...
          for (x=0; x < (long) bitmap->bitmap.width; x++)
          {
            ...
            if (active == MagickFalse)
              q=GetCacheViewPixels(image_view,x_offset,y_offset,1,1);
...
{
  (void) ThrowMagickException(&image->exception,GetMagickModule(),
    MissingDelegateWarning,"FreeTypeLibraryIsNotAvailable","`%s'",
    draw_info->font);
  return(RenderPostscript(image,draw_info,offset,metrics));
}
#endif
  ...
  if (draw_info->fill.opacity != TransparentOpacity)
    {
      ...
      */
      ...
      if (annotate_image->matte == MagickFalse)
        (void) SetImageOpacity(annotate_image,OpaqueOpacity);
      fill_color=draw_info->fill;
      annotate_view=OpenCacheView(annotate_image);
      for (y=0; y < (long) annotate_image->rows; y++)
      {
        q=GetCacheViewPixels(annotate_view,0,y,annotate_image->columns,1);
        if (q == (PixelPacket *) NULL)
          break;
