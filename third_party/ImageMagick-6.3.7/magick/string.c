...
MagickExport long FormatMagickSize(const MagickSizeType size,char *format)
{
  ...
  static const char
    *units[7] = { "b", "kb", "mb", "gb", "tb", "pb", "eb" };

  length=(double) ((MagickOffsetType) (size/2))*2.0;
  for (i=0; (length > 1024.0) && (i < 6); i++)
    length/=1024.0;
  return(FormatMagickString(format,MaxTextExtent,"%g%s",length,units[i]));
}

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  const char *format,va_list operands)
{
  ...
  locale=ConstantString(setlocale(LC_ALL,(const char *) NULL));
  ...
  char *timestamp)
{
  ...
  count=FormatMagickString(timestamp,length,
    ...
    local_meridian.tm_min,local_meridian.tm_sec,(long) offset);
  return(count);
}

/*
  ...
  const char *search,const char *replace)
{
  ...
  */
  ...
  (void) ConcatenateMagickString(result+destination_offset,source,(size_t)
    (length+MaxTextExtent-destination_offset)*sizeof(*result));
