...
%                                                                             %
%                                                                             %
%                     ImageMagick Image Threshold Methods                     %
%                                                                             %
%                               Software Design                               %
%                                 John Cristy                                 %
%                                 October 1996                                %
    ...
    if (channel == DefaultChannels)
      {
        ...
        {
          q->red=(Quantum) ((MagickRealType)
            PixelIntensityToQuantum(q) <= threshold ? 0 : QuantumRange);
          q->green=q->red;
          q->blue=q->red;
          q++;
        }
        ...
        if ((channel & OpacityChannel) != 0)
          {
            if (image->matte == MagickFalse)
              q->opacity=(Quantum) ((MagickRealType)
                q->opacity <= threshold ? 0 : QuantumRange);
            else
              q->opacity=(Quantum) ((MagickRealType)
...
MagickExport ThresholdMap *DestroyThresholdMap(ThresholdMap *map)
{
  ...
  if ( map && map->levels )
    map->levels = (int *) RelinquishMagickMemory(map->levels);
  if ( map )
  ...
  const char *filename,const char *map_id,ExceptionInfo *exception)
{
  ...
  /* Allocate the theshold levels array */
  ...
  map->levels=(int *) AcquireQuantumMemory((size_t) map->width,map->height*
    sizeof(*map->levels));
  if ( map->levels == (int *)NULL )
    ThrowFatalException(ResourceLimitFatalError,"UnableToAcquireThresholdMap");
  { /* parse levels into integer array */
    ...
    for( i=0; i<map->width*map->height; i++) {
      ...
      if ( map->levels[i] < 0 || map->levels[i] > map->divisor ) {
        (void) ThrowMagickException(exception,GetMagickModule(),OptionError,
          "XmlInvalidContent", "<level> %d out of range, map \"%s\"",
          map->levels[i], map_id);
        thresholds = DestroyXMLTree(thresholds);
        map = DestroyThresholdMap(map);
        return(map);
...
#define DitherImageTag  "Dither/Image"
  ...
  /* Set channel levels from extra comma seperated arguments
     Default to 2, or single value given, or individual channel values
  */
#if 1
  { /* parse directly as a comma seperated list of integers */
    ...
    if ( p != (char *)NULL && isdigit((int) ((unsigned char) *(++p))) )
      levels.index = 1UL*strtol(p, &p, 10);
    else
      levels.index = 2;
  ...
  { /* Do the posterized ordered dithering of the image */
    ...
    for (y=0; y < (long) image->rows; y++)
    {
      ...
      if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        ...
        {
          MagickBooleanType
            status=image->progress_monitor(DitherImageTag,y,image->rows,
               image->client_data);
          if (status == MagickFalse)
            break;
        }
...
#define ThresholdImageTag  "Threshold/Image"
  ...
  if (strchr(thresholds,'%') != (char *) NULL)
    ...
    if ((min_threshold <= 8) && ((max_threshold == min_threshold) ||
        (max_threshold == 1)))
      {
        /*
