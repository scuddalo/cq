...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  IsTGA() returns MagickTrue if the image format type, identified by the
%  magick string, is a Microsoft Windows Enhanced MetaFile (TGA) file.
%
%  The format of the ReadTGAImage method is:
%
%      MagickBooleanType IsTGA(const unsigned char *magick,const size_t length)
%
...
%
*/
static MagickBooleanType IsTGA(const unsigned char *magick,const size_t length)
{
  if (length < 4)
    return(MagickFalse);
  if (memcmp(magick,"\000\000\002\000",4) == 0)
    return(MagickTrue);
  return(MagickFalse);
}

          ...
          switch (tga_info.colormap_size)
          {
            ...
            case 16:
            {
              ...
              */
              ...
              pixel.red=ScaleAnyToQuantum(1UL*(k & 0x7c) >> 2,5);
              pixel.green=ScaleAnyToQuantum((1UL*(k & 0x03) << 3)+
                (1UL*(j & 0xe0) >> 5),5);
              pixel.blue=ScaleAnyToQuantum(1UL*(j & 0x1f),5);
              break;
            }
            case 24:
            case 32:
...
ModuleExport unsigned long RegisterTGAImage(void)
{
  ...
  entry->magick=(IsImageFormatHandler *) IsTGA;
