...
#if defined(MAGICKCORE_WMF_DELEGATE) || defined(MAGICKCORE_WMF_DELEGATElite)
...
#if defined(MAGICKCORE_WMF_DELEGATElite)
...
int magick_progress_callback(void *context,float quantum)
{
  ...
  if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
      (QuantumTick(offset,span) != MagickFalse))
    status=image->progress_monitor(LoadImagesTag,offset,span,
      image->client_data);
...
static void ipa_bmp_draw(wmfAPI *API, wmfBMP_Draw_t *bmp_draw)
{
       ...
       ddata->image_info->background_color.opacity != OpaqueOpacity )
  {
    ...
    */
    ...
    PaintTransparentImage( image, &white, QuantumRange );
  }
...
static void ipa_device_open(wmfAPI * API)
{
  ...
  ddata->draw_wand = DrawAllocateWand(draw_info,ddata->image);
      ...
      if (finish == magick_arc_ellipse)
      ...
      else if (finish == magick_arc_pie)
        {
          ...
          DrawPathMoveToAbsolute(WmfDrawingWand, XC(O.x+start.x), 
            YC(O.y+start.y));
          DrawPathEllipticArcAbsolute(WmfDrawingWand, Rx, Ry, 0, 0, 1, 
...
static void util_set_pen(wmfAPI * API, wmfDC * dc)
{
  ...
  DrawSetStrokeAntialias(WmfDrawingWand, 1 );
...
#endif /* MAGICKCORE_WMF_DELEGATElite */
...
#if defined(MAGICKCORE_WMF_DELEGATElite)  /* Only in libwmf 0.3 */
    printf("dc->Ox            = %.d\n", dc->Ox);
    printf("dc->Oy            = %.d\n", dc->Oy);
    printf("dc->width         = %.d\n", dc->width);
    printf("dc->height        = %.d\n", dc->height);
...
/* #endif */
#endif /* MAGICKCORE_WMF_DELEGATE || MAGICKCORE_WMF_DELEGATElite */

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                             %
