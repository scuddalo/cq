...
#define XMLProfile  "begin_xml_packet:"
    ...
    if (LocaleNCompare(XMLProfile,command,strlen(XMLProfile)) == 0)
      {
        ...
        /*
          Read XML profile.
        */
        count=(ssize_t) sscanf(command,XMLProfile " %lu",&extent);
        if (count != 1)
          continue;
        length=extent;
        profile=AcquireStringInfo(length);
        ...
        (void) SetImageProfile(image,"xml",profile);
        profile=DestroyStringInfo(profile);
        continue;
      }
    /*
  ...
  */
  ...
  (void) write(file,command,(unsigned int) strlen(command));
  ...
  *read_info->magick='\0';
  ...
  do
  {
    ...
    postscript_image->scene=scene++;
    (void) CloneImageProfiles(postscript_image,image);
    (void) CloneImageProperties(postscript_image,image);
    next_image=SyncNextImageInList(postscript_image);
    if (next_image != (Image *) NULL)
      postscript_image=next_image;
  ...
  do
  {
    ...
    if (page == 1)
      {
        ...
        */
        ...
        profile=GetImageProfile(image,"xml");
        if (profile != (StringInfo *) NULL)
          {
            /*
              Embed XML profile.
    ...
    */
    ...
    if ((image_info->type != TrueColorType) &&
        ((type == BilevelType) || (type == GrayscaleType) ||
         (type == GrayscaleMatteType)))
      {
        (void) FormatMagickString(buffer,MaxTextExtent,"%lu %lu\n1\n1\n1\n%d\n",
          image->columns,image->rows,type == BilevelType ? 1 : 8);
        (void) WriteBlobString(image,buffer);
        if (type != BilevelType)
          {
            /*
      ...
      if ((image->storage_class == DirectClass) ||
          (image->matte != MagickFalse))
        {
          ...
          */
          (void) FormatMagickString(buffer,MaxTextExtent,"%lu %lu\n0\n%d\n",
            image->columns,image->rows,
            image->compression == RLECompression ? 1 : 0);
          (void) WriteBlobString(image,buffer);
              ...
              */
              ...
              for (y=0; y < (long) image->rows; y++)
              {
                ...
                for (x=0; x < (long) image->columns; x++)
                {
                  if ((image->matte != MagickFalse) &&
                      (p->opacity == (Quantum) TransparentOpacity))
                    (void) CopyMagickString(buffer,"ffffff",MaxTextExtent);
                  else
                    (void) FormatMagickString(buffer,MaxTextExtent,
              ...
              */
              ...
              for (y=0; y < (long) image->rows; y++)
              {
                ...
                (void) FormatMagickString(buffer,MaxTextExtent,"%02lX%02lX",
                  (unsigned long) index,(unsigned long) MagickMin(length,0xff));
                (void) WriteBlobString(image,buffer);
                if (image->previous == (Image *) NULL)
                if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
                    (QuantumTick(y,image->rows) != MagickFalse))
