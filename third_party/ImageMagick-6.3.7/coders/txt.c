...
static Image *ReadTXTImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  *colorspace='\0';
  ...
  if (count != 4)
    ThrowReaderException(CorruptImageError,"ImproperImageHeader");
  if (max_value == 4294967295UL)
    image->depth=32;
  else
    for (image->depth=1; (1UL << image->depth)-1 < max_value; image->depth++);
  ...
  while (ReadBlobString(image,text) != (char *) NULL)
  {
    ...
    if (q == (PixelPacket *) NULL)
      continue;
    q->red=ScaleAnyToQuantum(pixel.red,image->depth);
    q->green=ScaleAnyToQuantum(pixel.green,image->depth);
    q->blue=ScaleAnyToQuantum(pixel.blue,image->depth);
    if (image->colorspace == CMYKColorspace)
      {
        indexes=GetIndexes(image);
        *indexes=ScaleAnyToQuantum(pixel.index,image->depth);
      }
    if (image->matte != MagickFalse)
      q->opacity=(Quantum) ScaleAnyToQuantum(pixel.opacity,image->depth);
...
static MagickBooleanType WriteTXTImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  (void) FormatMagickString(buffer,MaxTextExtent,
    ...
    image->rows,image->depth == 32 ? 4294967295UL : (1UL << image->depth)-1,
    colorspace);
