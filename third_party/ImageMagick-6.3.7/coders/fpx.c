...
static Image *ReadFPXImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  */
  scanline=(unsigned char *) AcquireQuantumMemory(image->columns,
    (tile_height+1UL)*colorspace.numberOfComponents*sizeof(*scanline));
...
static MagickBooleanType WriteFPXImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  pixels=(unsigned char *) AcquireQuantumMemory(image->columns,
    colorspace.numberOfComponents*sizeof(*pixels));
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    if (fpx_info.numberOfComponents == 1)
      ...
      if (image->matte == MagickFalse)
        ...
        (void) ImportQuantumPixels(image,&quantum_info,RGBAQuantum,pixels);
