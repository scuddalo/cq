...
static Image *ReadDCMImage(const ImageInfo *image_info,ExceptionInfo *exception)
{
  ...
  *transfer_syntax='\0';
    ...
    */
    if ((explicit_file == MagickFalse) && (group != 0x0002))
      explicit_file=(isupper((int) *explicit_vr) != MagickFalse) &&
        ...
        MagickFalse;
    use_explicit=(group == 0x0002) || (explicit_file != MagickFalse) ?
        ...
        switch (element)
        {
          case 0x0002:
          {
            /*
              Samples per pixel.
            */
            samples_per_pixel=(unsigned long) datum;
            break;
          }
          case 0x0004:
          {
          ...
          case 0x0100:
          {
            /*
              Bits allocated.
            */
            bits_allocated=(unsigned long) datum;
            bytes_per_pixel=1;
            if (datum > 8)
              bytes_per_pixel=2;
          ...
          case 0x0101:
          {
            /*
              Bits stored.
            */
            significant_bits=(unsigned long) datum;
            bytes_per_pixel=1;
          ...
          case 0x1203:
          {
            ...
            /*
              Initialize colormap.
            */
            status=AllocateImageColormap(image,(unsigned long) length/2);
            if (status == MagickFalse)
            ...
            if ((i != (long) length) && (length <= 4))
              {
                ...
                for (j=(long) length-1; j >= 0; j--)
                  datum=(256*datum+(long) data[j]);
                (void) fprintf(stdout,"%ld",datum);
              }
            else
  ...
  if (signed_data == 0xffff)
    signed_data=significant_bits == 16 ? 1 : 0;
  ...
  if (depth != (1UL*MAGICKCORE_QUANTUM_DEPTH))
    {
      ...
      */
      ...
      for (i=0; i < (long) (1UL << depth); i++)
        scale[i]=ScaleAnyToQuantum((unsigned long) i,depth);
    }
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          for (x=0; x < (long) image->columns; x++)
          {
            if (samples_per_pixel == 1)
              {
                if (bytes_per_pixel == 1)
                  index=(unsigned long) ReadBlobByte(image);
                else
                  if ((bits_allocated != 12) || (significant_bits != 12))
                    ...
                    {
                      if ((i & 0x01) != 0)
                        index=((unsigned long) ReadBlobByte(image) << 8) | byte;
                      else
                        {
                          if (msb_first != MagickFalse)
