...
static void MngInfoDiscardObject(MngInfo *mng_info,int i)
{
  if (i && (i < MNG_MAX_OBJECTS) && (mng_info != (MngInfo *) NULL) &&
      mng_info->exists[i] && !mng_info->frozen[i])
    {
      ...
      mng_info->object_clip[i].right=(long) PNG_MAX_UINT;
      mng_info->object_clip[i].top=0;
      mng_info->object_clip[i].bottom=(long) PNG_MAX_UINT;
    }
}
  ...
  if (ping_info->valid & PNG_INFO_tRNS)
    {
      ...
      scale=(int) (QuantumRange/((1L << ping_info->bit_depth)-1L));
      if (scale < 1)
         scale=1;
      /*
        Image has a transparent background.
      */
      transparent_color.red=(Quantum) (ping_info->trans_values.red*scale);
      transparent_color.green=(Quantum) (ping_info->trans_values.green*scale);
      transparent_color.blue=(Quantum) (ping_info->trans_values.blue*scale);
      transparent_color.opacity=(Quantum) (ping_info->trans_values.gray*scale);
      ...
      */
      ...
      for (y=0; y < (long) image->rows; y++)
      {
        ...
        if (q == (PixelPacket *) NULL)
          break;
#if ((MAGICKCORE_QUANTUM_DEPTH == 8) && !defined(MAGICKCORE_HDRI_SUPPORT))
        if (depth == 16)
          {
            register Quantum
              *p,
        ...
        if (depth == 8 && ping_info->color_type == PNG_COLOR_TYPE_GRAY)
          (void) ExportQuantumPixels(image,&quantum_info,GrayQuantum,
            png_pixels+row_offset);
        if (ping_info->color_type == PNG_COLOR_TYPE_GRAY ||
            ping_info->color_type == PNG_COLOR_TYPE_GRAY_ALPHA)
          {
            image->depth=8;
            (void) ExportQuantumPixels(image,&quantum_info,GrayAlphaQuantum,
              png_pixels+row_offset);
            image->depth=depth;
          }
              ...
              ping_info->color_type == PNG_COLOR_TYPE_RGB_ALPHA)
          {
            ...
            (void) ExportQuantumPixels(image,&quantum_info,IndexQuantum,
              png_pixels+row_offset);
      ...
      if (image->previous == (Image *) NULL)
        if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
            (QuantumTick(pass,(MagickSizeType) num_passes) != MagickFalse))
          {
            status=image->progress_monitor(LoadImageTag,pass,(MagickSizeType)
              num_passes,image->client_data);
      ...
      if ((mng_info->ob[object_id] == (MngBuffer *) NULL) ||
      ...
      else
        {
          ...
          if (mng_info->ob[object_id]->image != (Image *) NULL)
            mng_info->ob[object_id]->image=DestroyImage(mng_info->ob[object_id]->image);
    ...
    const ImageInfo *image_info, ExceptionInfo *exception)
{
  ...
  if (GetPixels(image) != (PixelPacket *) NULL)
    {
      ...
      */
      if (logging != MagickFalse)
        (void) LogMagickEvent(CoderEvent,GetMagickModule(),"  AllocateNextImage()");
    ...
    if (image->progress_monitor != (MagickProgressMonitor) NULL)
      {
        status=image->progress_monitor(LoadImagesTag,TellBlob(image),
          2*GetBlobSize(image),image->client_data);
        if (status == MagickFalse)
          break;
      }
    type[0]='\0';
            ...
            if (jng_alpha_compression_method == 0)
              {
                ...
                (void) WriteBlob(alpha_image,8,(unsigned char *)
                  "\211PNG\r\n\032\n");
  ...
  */
  ...
  mng_info->image_found++;
  if (image->progress_monitor != (MagickProgressMonitor) NULL)
    status=image->progress_monitor(LoadImageTag,2*TellBlob(image),
      2*GetBlobSize(image),image->client_data);
  if (logging != MagickFalse)
    (void) LogMagickEvent(CoderEvent,GetMagickModule(),
        ...
        if (memcmp(type,mng_FRAM,4) == 0)
          {
                ...
                if ((p-chunk) < (long) (length-4))
                  {
                    ...
                    if (change_delay)
                      {
                        frame_delay=(1UL*image->ticks_per_second*(mng_get_long(p))/
                            mng_info->ticks_per_second);
                        if (change_delay == 2)
                    ...
                    if (change_timeout)
                      {
                        frame_timeout=
                            (1UL*image->ticks_per_second*(mng_get_long(p))/mng_info->ticks_per_second);
                        if (change_delay == 2)
            ...
            subframe_width=(unsigned long) (mng_info->clip.right-mng_info->clip.left);
            subframe_height=(unsigned long) (mng_info->clip.bottom-mng_info->clip.top);
            /*
        ...
        if (memcmp(type,mng_pHYs,4) == 0)
          {
            if (length > 8)
              {
                mng_info->global_x_pixels_per_unit=(unsigned long) mng_get_long(p);
                mng_info->global_y_pixels_per_unit=(unsigned long) mng_get_long(&p[4]);
...
#define QM Quantum
                ...
                for (y=0; y < (long) image->rows; y++)
                {
                  ...
                  for (i=0; i < m; i++, yy++)
                  {
                    ...
                    if (SyncImagePixels(large_image) == 0)
                      break;
                  } /* i */
                } /* y */
    ...
    if (mng_info->optimize && image->depth == 16)
      {
        ...
        ok_to_reduce=(((((unsigned long) image->background_color.red >> 8) & 0xff)
        ...
        if (ok_to_reduce && image->storage_class == PseudoClass)
          {
            ...
            for (index=0; index < (long) image->colors; index++)
              {
                ok_to_reduce=(((((unsigned long) image->colormap[index].red >> 8) & 0xff)
                  == ((unsigned long) image->colormap[index].red & 0xff)) &&
                  ((((unsigned long) image->colormap[index].green >> 8) & 0xff)
                  == ((unsigned long) image->colormap[index].green & 0xff)) &&
                  ((((unsigned long) image->colormap[index].blue >> 8) & 0xff)
                  == ((unsigned long) image->colormap[index].blue & 0xff)));
        ...
        if ((ok_to_reduce != MagickFalse) && (image->storage_class != PseudoClass))
          {
            ...
            for (y=0; y < (long) image->rows; y++)
            {
              ...
              for (x=(long) image->columns-1; x >= 0; x--)
              {
                ok_to_reduce=(((((unsigned long) p->red >> 8) & 0xff) == ((unsigned long) p->red & 0xff)) &&
                  ((((unsigned long) p->green >> 8) & 0xff) == ((unsigned long) p->green & 0xff)) &&
                  ((((unsigned long) p->blue >> 8) & 0xff) == ((unsigned long) p->blue & 0xff)) &&
                  (((!image->matte ||
                  (((unsigned long) p->opacity >> 8) & 0xff) == ((unsigned long) p->opacity & 0xff)))));
                if (ok_to_reduce == 0)
                  break;
                p++;
              }
  ...
  if (mng_info->ticks_per_second && mng_info->image_found > 1 && GetNextImageInList(image) ==
     (Image *) NULL)
    {
      if (logging != MagickFalse)
  ...
  if (mng_info->ticks_per_second)
    final_delay=1UL*MagickMax(image->ticks_per_second,1L)*final_delay/mng_info->ticks_per_second;
  else
    image->start_loop=MagickTrue;
  ...
  if (insert_layers)
    {
      ...
      if (next_image == (Image *) NULL)
        ThrowFatalException(image->exception.severity,
...
ModuleExport unsigned long RegisterPNGImage(void)
{
  ...
  *version='\0';
  ...
  if (LocaleCompare(PNG_LIBPNG_VER_STRING,png_get_header_ver(NULL)) != 0)
    {
      ...
      (void) ConcatenateMagickString(version,png_get_libpng_ver(NULL),MaxTextExtent);
    }
#endif
#endif
  ...
  entry->magick=(IsImageFormatHandler *) IsPNG;
  entry->adjoin=MagickFalse;
  entry->description=ConstantString("8-bit indexed with optional binary transparency");
  entry->module=ConstantString("PNG");
  (void) RegisterMagickInfo(entry);
...
ModuleExport void UnregisterPNGImage(void)
{
  ...
  AcquireSemaphoreInfo(&png_semaphore);
  RelinquishSemaphoreInfo(png_semaphore);
  png_semaphore=DestroySemaphoreInfo(png_semaphore);
#endif
}

#if defined(MAGICKCORE_PNG_DELEGATE)
   ...
   *profile_description, unsigned char *profile_data, png_uint_32 length)
{
   ...
   text[0].key[0]='\0';
   (void) ConcatenateMagickString(text[0].key, "Raw profile type ",MaxTextExtent);
   ...
   *dp++='\n';
   (void) FormatMagickString(dp,allocated_length-(png_size_t) (dp-text[0].text),"%8lu ",length);
   dp+=8;
   for (i=0; i < (long) length; i++)
   {
     if (i%36 == 0)
...
# endif
  ...
  */
  ...
  if (mng_info->write_png8)
          ...
          if ((image->storage_class == DirectClass) || (number_colors > 256))
            {
              ...
              quantize_info.number_colors= (matte != MagickFalse ? 255UL : 256UL);
          ...
          */
...
#endif
          palette=(png_color *) AcquireQuantumMemory((number_colors+1),
            sizeof(*palette));
          ...
          palette=(png_colorp) RelinquishMagickMemory(palette);
            ...
            if (matte)
            {
              ...
              */
              ping_info->trans=(unsigned char *) AcquireQuantumMemory(
                number_colors,sizeof(*ping_info->trans));
                  ...
                  if (p->opacity != OpaqueOpacity)
                    {
                      ...
                      ping_info->trans[(long) indexes[x]]=(png_byte) (255-
              ...
              if (ping_info->num_trans == 0)
                ping_info->trans=(unsigned char *)
                  RelinquishMagickMemory(ping_info->trans);
            }
          /*
  ...
  else
    {
      ...
      image->depth=ping_info->bit_depth;
      ping_info->color_type=(png_byte) ((matte != MagickFalse)?PNG_COLOR_TYPE_RGB_ALPHA:PNG_COLOR_TYPE_RGB);
              ...
              */
              for (y=0; y < (long) image->rows; y++)
              {
                ...
                for (x=(long) image->columns-1; x >= 0; x--)
                {
                  if (p->opacity != OpaqueOpacity)
                    {
                      if (IsPNGColorEqual(ping_info->trans_values,*p) == 0)
                      {
                    ...
                    {
                      if (IsPNGColorEqual(ping_info->trans_values,*p))
    ...
    if ((mng_info->optimize || mng_info->IsPalette) &&
        ImageIsGray(image) && (!image->matte || image->depth >= 8))
      {
        ...
        if (image->colors == 0 || image->colors-1 > MaxColormapSize)
            ...
            if ((int) ping_info->color_type == PNG_COLOR_TYPE_PALETTE)
              {
                ...
                PNG_COLOR_TYPE_GRAY && image->colors < 17 && mng_info->IsPalette)
              {
                  ...
                  if (p->opacity != OpaqueOpacity)
                    {
                      ...
                      if (trans[(long) index] != 256)
                        {
                          if (trans[(long) index] != (png_byte) (255-
                             ScaleQuantumToChar(p->opacity)))
                            {
    ...
    */
    if (ping_info->bit_depth < 8 && ping_info->color_type ==
        PNG_COLOR_TYPE_GRAY)
      {
         ...
         ping_info->trans_values.gray=(png_uint_16) (QuantumScale*(maxval*
  ...
  */
  ...
  quality=image->quality == UndefinedCompressionQuality ? 75UL : image->quality;
  if (quality > 9)
    {
      int
        level;
    ...
    if (profile != (StringInfo *) NULL)
      {
        ...
        if ((LocaleCompare(name,"ICC") == 0) ||
          ...
          png_set_iCCP(ping,ping_info,(png_charp) name,0,(png_charp)
            GetStringInfoDatum(profile),(png_uint_32) GetStringInfoLength(profile));
        else
#endif
  ...
  not_valid=(unsigned int) (!ping_info->valid);
...
#undef PNG_HAVE_IDAT
  ...
  */
  ...
  if ((!mng_info->write_png8 && !mng_info->write_png24 && !mng_info->write_png32) &&
        ...
        */
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          for (i=0; i < (long) image->columns; i++)
             *(png_pixels+i)=(unsigned char) ((*(png_pixels+i) > 127) ? 255 : 0);
    ...
    {
      if ((!mng_info->write_png8 && !mng_info->write_png24 && !mng_info->write_png32) &&
         (!image->matte || (ping_info->bit_depth >= MAGICKCORE_QUANTUM_DEPTH)) &&
         (mng_info->optimize || mng_info->IsPalette) && ImageIsGray(image))
      {
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          if (ping_info->color_type == PNG_COLOR_TYPE_GRAY)
            ...
            {
              (void) ImportQuantumPixels(image,&quantum_info,GrayAlphaQuantum,
                png_pixels);
            }
          png_write_row(ping,png_pixels);
        }
      ...
      for (pass=0; pass < num_passes; pass++)
      {
        if ((image->depth > 8) || (mng_info->write_png24 || mng_info->write_png32 ||
    ...
    if (value != (const char *) NULL)
      {
        ...
        text[0].compression=image->compression == NoCompression ||
 ...
 * 1.0.5a is leaky */
        ...
        ping_info->text[i].compression=image->compression == NoCompression ||
          (image->compression == UndefinedCompression &&
      ...
      if (mng_info->page.x || mng_info->page.y || (ping_info->width != mng_info->page.width) ||
          (ping_info->height != mng_info->page.height))
        {
          ...
          */
          ...
          PNGLong(chunk+27,(png_uint_32) (mng_info->page.y + ping_info->height));
  ...
  (void) SetImageProperty(image,"png:bit-depth",s);
  ...
  */
  ...
  if (ping_info->valid & PNG_INFO_tRNS)
    {
      ping_info->trans=(unsigned char *) RelinquishMagickMemory(ping_info->trans);
      ping_info->valid&=(~PNG_INFO_tRNS);
    }
  png_destroy_write_struct(&ping,&ping_info);
      ...
      if (jng_alpha_compression_method==0)
        {
          ...
          value=GetImageProperty(jpeg_image,"png:bit-depth");
          if (value != (char *) NULL)
...
static MagickBooleanType WriteJNGImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  (void) WriteBlob(image,8,(unsigned char *) "\213JNG\r\n\032\n");
...
static MagickBooleanType WriteMNGImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  mng_info->adjoin=image_info->adjoin && (GetNextImageInList(image) != (Image *) NULL) &&
    write_mng;
  ...
  */
  ...
  if (write_mng)
    ...
    defined(PNG_MNG_FEATURES_SUPPORTED)
      ...
      for (next_image=image; next_image != (Image *) NULL; )
      {
        ...
        if (final_delay != initial_delay || final_delay > 1UL*next_image->ticks_per_second)
     ...
     if (mng_info->need_fram == MagickFalse)
       {
         ...
         */
         ...
         if (need_defi && final_delay > 2 && (final_delay != 4) &&
            ...
            (final_delay != 25) && (final_delay != 50) && (final_delay != 1UL*image->ticks_per_second))
           mng_info->need_fram=MagickTrue;  /* make it exact; cannot be VLC anyway */
       }
     if (mng_info->need_fram != MagickFalse)
        mng_info->ticks_per_second=1UL*image->ticks_per_second;
     /*
     ...
     */
     (void) WriteBlob(image,8,(unsigned char *) "\212MNG\r\n\032\n");
     ...
     if ((GetPreviousImageInList(image) == (Image *) NULL) &&
         ...
         (image->iterations != 1))
       {
         ...
         */
         ...
         PNGLong(chunk+6,(png_uint_32) (mng_info->ticks_per_second*final_delay/MagickMax(image->ticks_per_second,1)));
         ...
         if (logging != MagickFalse)
           {
             (void) LogMagickEvent(CoderEvent,GetMagickModule(),
               "     TERM delay: %lu",
               (png_uint_32) (mng_info->ticks_per_second*final_delay/MagickMax(image->ticks_per_second,1)));
             if (image->iterations == 0)
               (void) LogMagickEvent(CoderEvent,GetMagickModule(),
     ...
     */
     if (write_mng && (image->matte || image->page.x > 0 || image->page.y > 0 ||
         (image->page.width && (image->page.width+image->page.x < mng_info->page.width))
...
#ifdef PNG_WRITE_EMPTY_PLTE_SUPPORTED
     if ((need_local_plte == MagickFalse) && (image->storage_class == PseudoClass) &&
         (all_images_are_gray == MagickFalse))
       {
         unsigned long
           data_length;
  ...
  do
  {
    ...
    if (need_defi)
      {
        ...
        mng_info->page=image->page;
        if ((mng_info->page.x !=  previous_x) || (mng_info->page.y != previous_y))
          {
       ...
       else
         {
           ...
           */
           ...
           PNGLong(chunk+10,(png_uint_32)
             ((mng_info->ticks_per_second*image->delay)/MagickMax(image->ticks_per_second,1)));
           (void) WriteBlob(image,14,chunk);
           (void) WriteBlobMSBULong(image,crc32(0,chunk,14));
   ...
   if (image->compression == JPEGCompression)
    ...
    if (image->progress_monitor != (MagickProgressMonitor) NULL)
      {
        status=image->progress_monitor(SaveImagesTag,(MagickOffsetType) scene,
