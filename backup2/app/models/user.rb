...
class User < ActiveRecord::Base
  ...
  validates_each(:email, :on => :create, :allow_blank => true) do |user, attribute, value|
    logger.info("@@@@@ validates_each block in user.rb: #{user}, #{attribute} #{value}")
    p = Profile.find_by_email(value)
    user.errors.add(attribute, 'is already taken') unless p.blank?
  end
  after_create do |user|
    logger.info("@@@@@ after_create in user: creating profile after creating user with email...#{user.email}")
    ...
    user.profile = p
    if !p.save
      logger.info("oops!...profile.save failed.")
      fail p.errors.to_s
    end
    logger.info("@@@@ ====== after saving profile....profile_id is : #{p.id} and it belong to user: #{p.user.id}")
  end
